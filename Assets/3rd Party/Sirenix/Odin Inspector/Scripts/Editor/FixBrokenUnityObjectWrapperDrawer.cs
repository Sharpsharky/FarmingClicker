...
namespace _3rd_Party.Sirenix.Odin_Inspector.Scripts.Editor
{
    ...
    public class FixBrokenUnityObjectWrapperDrawer<T> : OdinValueDrawer<T>, IDefinesGenericMenuItems
        where T : Component
    {
        private const string AUTO_FIX_PREFS_KEY =
            "TemporarilyBrokenUnityObjectWrapperDrawer.autoFix";
        ...
        public void PopulateGenericMenu(InspectorProperty property, GenericMenu genericMenu)
        {
            ...
            {
                genericMenu
                    .AddItem(new GUIContent("Disable auto-fix of broken prefab instance references"),
                             false, x =>
                                    {
            ...
            if(Event.current.type == EventType.Layout)
            {
                ...
                if(isBroken && autoFix)
                {
                    ...
                    for(int i = 0; i < ValueEntry.ValueCount; i++)
                    {
                        ...
                        if(ComponentIsBroken(ValueEntry.Values[i], ref fixedComponent) &&
                           fixedComponent)
                        {
                            (ValueEntry as IValueEntryActualValueSetter<T>)
                                .SetActualValue(i, fixedComponent);
                        }
                    }
            ...
            if(oObj != null && uObj == null)
            {
                ...
                if(AssetDatabase.Contains(instanceId))
                {
                    ...
                    var realWrapper = AssetDatabase.LoadAllAssetsAtPath(path)
                                                   .FirstOrDefault(n => n.GetInstanceID() ==
                                                       instanceId) as T;
                    if(realWrapper)
                    {
            ...
            public void FixItThisTime()
            {
                for(int i = 0; i < valueEntry.ValueCount; i++)
                {
                       ...
                       fixedComponent)
                    {
                        ...
                        {
                            (valueEntry as IValueEntryActualValueSetter<T>)
                                .SetActualValue(localI, fixedComponent);
                        });
                    }
                }
