...
namespace Utils.ScreenshotUtility.Editor
{
    public sealed class ScreenshotUtility : EditorWindow, IHasCustomMenu
    {
        ...
        private static readonly string[] aspectList = {"16:9", "21:9", "32:9"};

        private static readonly string[] DateFormats = {"MM-dd-yyyy", "dd-MM-yyyy", "yyyy-MM-dd"};

        private static readonly string[] TimeFormats = {"24 Hour", "AM-PM"};
        ...
        {
            get => EditorPrefs.GetString(PlayerSettings.productName + "_SRCSHOT_FileNameFormat",
                                         "{S}_{R}_{D}_{T}");
        ...
        private void OnGUI()
        {
            ...
            {
                EditorGUILayout
                    .HelpBox("No active camera's found (or they're hidden/have a render texture)",
                             MessageType.Warning);
                return;
            }
                ...
                EditorGUILayout.BeginHorizontal();
                {
                    if(GUILayout.Toggle(captureScene,
                                        new GUIContent("Scene",
                                                       EditorGUIUtility
                                                           .IconContent("d_unityeditor.sceneview.png")
                                                           .image), EditorStyles.miniButtonLeft))
                    {
                        captureScene = true;
                        captureGame = false;
                    }
                    ...
                    if(GUILayout.Toggle(captureGame,
                                        new GUIContent(" Game",
                                                       EditorGUIUtility
                                                           .IconContent("unityeditor.gameview.png")
                                                           .image), EditorStyles.miniButtonRight))
                    {
                        captureGame = true;
                        captureScene = false;
                    }
            ...
            EditorGUILayout.BeginVertical(EditorStyles.helpBox);
            {
                EditorGUILayout.LabelField("Resolution (" + ssWidth + " x " + ssHeight + ")",
                                           EditorStyles.boldLabel);

                EditorGUILayout.BeginHorizontal();
                Resolution = EditorGUILayout.Popup(Resolution, reslist, GUILayout.MinWidth(75f),
                                                   GUILayout.MaxWidth(75f));
                //If not custom
                if(Resolution != reslist.Length - 1)
                {
                    AspectRatio = EditorGUILayout.Popup(AspectRatio, aspectList,
            ...
            EditorGUILayout.BeginVertical(EditorStyles.helpBox);
            {
                ...
                EditorGUILayout.BeginHorizontal();
                {
                    ...
                    if(GUILayout.Button(new GUIContent("...", "Browse"), EditorStyles.miniButtonLeft,
                                        GUILayout.Width(25f)))
                    {
                        SavePath =
                            EditorUtility.SaveFolderPanel("Screenshot destination folder", SavePath,
            ...
            EditorGUILayout.BeginVertical(EditorStyles.helpBox);
            {
                EditorGUI.BeginDisabledGroup(SavePath == string.Empty);
                {
                    ...
                    {
                        OutputPNG = GUILayout.Toggle(OutputPNG, "PNG", EditorStyles.miniButtonLeft,
                                                     GUILayout.Width(50f),
                                                     GUILayout.ExpandWidth(false));
                        OutputPNG = !GUILayout.Toggle(!OutputPNG, "JPG", EditorStyles.miniButtonRight,
                                                      GUILayout.Width(50f),
                                                      GUILayout.ExpandWidth(false));
                        OpenAfterCapture =
                            EditorGUILayout.ToggleLeft("Auto open", OpenAfterCapture,
                    ...
                    using(new EditorGUILayout.HorizontalScope())
                    {
                        if(GUILayout.Button(new GUIContent("- Capture -"),
                                            GUILayout.ExpandHeight(false), GUILayout.Height(30f)))
                        {
                            RenderScreenshot();
                        }
                    }
        ...
        public static string FormatFileName(int resIndex, string dateFormat)
        {
            ...
            fileName =
                fileName.Replace("{T}",
                                 DateTime.Now.ToString(TimeFormat == 0 ? "HH-mm-ss" : "hh-mm-ss tt"));
        ...
        private void RefreshCameras()
        {
            ...
            for(int i = 0; i < cameraNames.Length; i++)
            {
                ...
                if(cameras[i].CompareTag("MainCamera") && cameraID < 0)
                {
                    cameraID = i;
                }
        ...
        private void SetResolution()
        {
            ...
            //21:9
            if(AspectRatio == 1 && Resolution != reslist.Length - 1)
            {
                ssWidth = Mathf.RoundToInt(ssWidth * 1.3215f);
            }

            //32:9
            if(AspectRatio == 2 && Resolution != reslist.Length - 1)
            {
 ...
 sourceCamera.GetComponent<Cinemachine.CinemachineBrain>();
        ...
        if (cBrain) cBrainEnable = cBrain.enabled;
            ...
            EditorApplication.ExecuteMenuItem("Window/Scene");
                ...
                if (cBrain && cBrainEnable) cBrain.enabled = false;
                    ...
                    sourceCamera.orthographicSize =
                        SceneView.lastActiveSceneView.camera.orthographicSize;
            ...
            var rt = new RenderTexture(ssWidth, ssHeight, 16,
            ...
            var screenShot = new Texture2D(ssWidth, ssHeight,
            ...
            string filename = FormatFileName(Resolution, DateFormats[DateFormat]) +
            ...
            var provider = new SettingsProvider("Preferences/Screenshot", SettingsScope.User)
                           {
                               ...
                               guiHandler = searchContent =>
                                            {
                                                FileNameFormat =
                                                    EditorGUILayout.TextField("File name format",
                                                        FileNameFormat);
                                                EditorGUILayout
                                                    .HelpBox("{P} = Project name\n{S} = Scene name\n{R} = Resolution\n{D} = Date\n{T} = Time\n{U} = Unique number",
                                                             MessageType.None);
                                                DateFormat =
                                                    EditorGUILayout.Popup("Date format", DateFormat,
                                                        DateFormats, GUILayout.MaxWidth(250f));
                                                TimeFormat =
                                                    EditorGUILayout.Popup("Time format", TimeFormat,
                                                        TimeFormats, GUILayout.MaxWidth(250f));
                                                EditorGUILayout
                                                    .LabelField("Example: " + FormatFileName(1, DateFormats[DateFormat]),
                                                ...
                                                ListenToPrintButton =
                                                    EditorGUILayout
                                                        .Toggle(new GUIContent("Print-screen button", "If the window is open and the scene-view is active, capture a screenshot whenever the print-screen keyboard button is pressed"),
                                                                ListenToPrintButton);
                                            },
