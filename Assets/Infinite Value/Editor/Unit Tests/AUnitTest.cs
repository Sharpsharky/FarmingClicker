...
{
    public partial class UnitTestsWindow
    {
            ...
            protected dynamic P_CreateRandomValue(Type type = null)
            {
                ...
                switch (type.Name)
                {
                    ...
                    case "Single": 
                        ...
                        return (avoidSpecialValues && specialNumberStr.Contains(f.ToString(CultureInfo.InvariantCulture)) ? P_CreateRandomValue(type) : f);
                    case "Double":
                        double d = BitConverter.ToDouble(buf, 0);
                        return (avoidSpecialValues && specialNumberStr.Contains(d.ToString(CultureInfo.InvariantCulture)) ? P_CreateRandomValue(type) : d);
                }
