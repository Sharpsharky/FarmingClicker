...
namespace InfiniteValue
{
    ...
    public class InfValInputField : MonoBehaviour
    {
        ...
        Dictionary<Type, Action> _possibleTargetTypeToMethodsDico = null;
        IReadOnlyDictionary<Type, Action> possibleTargetTypeToMethodsDico
        {
            get
            {
                if (_possibleTargetTypeToMethodsDico == null)
                {
                    _possibleTargetTypeToMethodsDico = new Dictionary<Type, Action>
                    {
                ...
                return _possibleTargetTypeToMethodsDico;
            }
        }
        ...
        /// <summary>Wether the value is valid or not.</summary>
        public bool isValid 
        {
            get => _isValid; 
            private set
        ...
        public void FindTargetComponent()
        {
            if (_target == null)
            {
                Type[] types = possibleTargetTypeToMethodsDico.Keys.ToArray();
                for (int i = 0; i < types.Length && target == null; i++)
                    _target = GetComponent(types[i]);
            }

            if (_target != null)
            {
                possibleTargetTypeToMethodsDico[target.GetType()]();
        ...
        char ValidateCharacter(string text, int charIndex, char addedChar)
        {
            ...
            // check wether we are at the beggining of the text
            ...
            // return wether adding the char will make the string fail validation
